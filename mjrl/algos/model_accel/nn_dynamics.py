    ...
    def fit_dynamics(
        ...
        **kwargs,
        ...
        if set_transformations:
            ...
            s_scale, a_scale = (
                torch.mean(torch.abs(s - s_shift), dim=0),
    ...
    def fit_reward(
        ...
        **kwargs,
        ...
        if set_transformations:
            ...
            r_shift, r_scale = (
                torch.mean(r, dim=0),
                torch.mean(torch.abs(r - r_shift), dim=0),
            )
...
class DynamicsNet(nn.Module):
    ...
    def set_transformations(
        ...
        self.out_shift, self.out_scale = (
            self.out_shift.to(device),
            self.out_scale.to(device),
        )
    ...
    def set_transformations(
        ...
        if s_shift is None:
            self.s_shift, self.s_scale = (
                ...
                torch.ones(self.state_dim),
            )
            self.a_shift, self.a_scale = (
                torch.zeros(self.act_dim),
                torch.ones(self.act_dim),
            )
        ...
        self.a_shift, self.a_scale = self.a_shift.to(device), self.a_scale.to(device)
        self.sp_shift, self.sp_scale = (
            self.sp_shift.to(device),
            self.sp_scale.to(device),
        )
